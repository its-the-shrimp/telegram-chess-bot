...
def get_pgn_file(
    update: Update, context: base.BoardGameContext, args: list[str]
) -> None:
...
class BaseMatch:
    ...
    def __init__(
        ...
        self.date: Optional[str] = date or datetime.datetime.now().strftime(
            utils.DATE_FORMAT
        )
...
class GroupMatch(BaseMatch):
    ...
    def __bytes__(self) -> bytes:
        return parsers.CGNParser.encode(
            ...
            result=self.state,
            headers={
                "WUID": hex(self.player1.id)[2:].encode(),
                "BUID": hex(self.player2.id)[2:].encode(),
                "CID": hex(self.msg.chat_id)[2:].encode()
                if not isinstance(self.msg, base.InlineMessage)
                else b"0",
                "MID": hex(self.msg.message_id)[2:].encode()
                if isinstance(self.msg, Message)
                else self.msg.message_id.encode(),
                "MSGTXT": self.init_msg_text.encode() or b"",
                "OPTIONS": json.dumps(self.options).encode(),
            },
        )

    @classmethod
    def _from_cgn(
        cls, obj: parsers.MatchData, dispatcher: Dispatcher, id: str
    ) -> "GroupMatch":
        player1 = User(
        ...
        if obj["headers"]["CID"] == "0":
            msg = cast(
                Union[Message, base.InlineMessage],
                base.InlineMessage(obj["headers"]["MID"], dispatcher.bot),
            )
        else:
            msg = Message(
    ...
    def init_turn(self, move: core.Move = None) -> None:
        ...
        if self.state not in (core.GameState.NORMAL, core.GameState.CHECK):
            ...
            if self.state == core.GameState.CHECK:
                self.init_msg_text += (
                    langtable["opponent-in-check"].format(name=self.db.get_name(player))
                    + "\n"
                )
            ...
            self.msg = cast(
                ...
                self.msg.edit_media(
                    media=InputMediaPhoto(
                        base.get_temp_url(
                            media.board_image(
                                lang_code=player.language_code,
                                moves=self.moves,
                    ...
                    reply_markup=self._keyboard(
                        cast(list[_CallbackDataInput], keyboard),
    ...
    def _send_analysis_video(self):
        if self.state in (
            ...
            text = "\n".join(
                [
                    base.langtable[self.player1.language_code][
                        self.state.name.lower().replace("_", "-")
                    ].format(name=self.db.get_name(player)),
                    base.langtable[self.player1.language_code]["winner"].format(
                        name=self.db.get_name(opponent)
                    ),
                    base.langtable[self.player1.language_code]["n-moves"].format(
            ...
            text = "\n".join(
                [
                    ...
                    base.langtable[self.player1.language_code]["draw"],
                    base.langtable[self.player1.language_code]["n-moves"].format(
                        n=self.cur_board.turn - 1
                    ),
                ]
        ...
        try:
            ...
            video, thumb = media.board_video(self, player.language_code)
            logging.info("Analysis video created")

            new_msg = InputMediaVideo(
                base.get_temp_url(video, "video/mp4"),
                caption=text,
                thumb=base.get_temp_url(thumb, "image/jpeg"),
            )
            ...
            self.msg = cast(
                ...
                self.msg.edit_media(
                    ...
                    reply_markup=self._keyboard(
                        [
                            {
                                "text": base.langtable[player.language_code][
                                    "download-pgn"
                                ],
            ...
            text += "\n\n" + base.langtable[player.language_code]["visualization-error"]
            self.msg = cast(
                Union[Message, base.InlineMessage],
                self.msg.edit_caption(caption=text),
            )
    ...
    def send_analysis_video(self) -> None:
        title_text = base.langtable[self.player1.language_code][
            self.state.name.lower().replace("_", "-")
        ]
        if self.state in (
            core.GameState.WHITE_CHECKMATED,
            ...
            player, opponent = self.players
            title_text = title_text.format(name=self.db.get_name(player))

        eval_thread = threading.Thread(
            target=self._send_analysis_video, name=self.id + ":eval"
        )
        ...
        self.msg = cast(
            ...
            self.msg.edit_media(
                media=InputMediaPhoto(
                    base.get_temp_url(
                        media.board_image(
                            lang_code=self.player1.language_code,
                            moves=self.moves,
                            startpos=self.startpos,
                            player1_name=self.db.get_name(self.player1),
                            player2_name=self.db.get_name(self.player2),
                    ...
                    caption="\n".join(
                        [
                            title_text,
                            base.langtable[self.player1.language_code]["waiting-eval"],
                        ]
                    ),
                )
            ),
            ...
            if command == "OTHER":
                ...
                self.msg = cast(
                    ...
                    self.msg.edit_media(
                        ...
                        reply_markup=self._keyboard(
                            cast(list[_CallbackDataInput], piece_buttons),
                            expected_uid=player.id,
                            head_item=True,
                        ),
                    ),
                ...
                self.msg = cast(
                    ...
                    self.msg.edit_media(
                        ...
                        reply_markup=self._keyboard(
                            cast(list[_CallbackDataInput], dest_buttons),
                            expected_uid=player.id,
                            head_item=True,
                        ),
                    ),
                ...
                self.msg = cast(
                    ...
                    self.msg.edit_media(
                        ...
                        reply_markup=self._keyboard(
                            cast(list[_CallbackDataInput], pieces),
                            expected_uid=player.id,
                        ),
                    ),
                )
...
class PMMatch(BaseMatch):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        players = [(player1, msg1), (player2, msg2)]
        if shuffle:
            random.shuffle(players)
        (self.player1, self.msg1), (self.player2, self.msg2) = players
        super().__init__(**kwargs)
    ...
    def __bytes__(self) -> bytes:
        return parsers.CGNParser.encode(
            ...
            headers={
                ...
                else hex(self.player1.id)[2:].encode(),
                "BUID": b"BOT"
                if self.player2.is_bot
                else hex(self.player2.id)[2:].encode(),
                "CID1": hex(self.msg1.chat_id)[2:].encode() if self.msg1 else b"0",
                "CID2": hex(self.msg2.chat_id)[2:].encode() if self.msg2 else b"0",
                "MID1": hex(self.msg1.message_id)[2:].encode() if self.msg1 else b"0",
                "MID2": hex(self.msg2.message_id)[2:].encode() if self.msg2 else b"0",
                "MSGTXT": self.init_msg_text.encode(),
                "OPTIONS": json.dumps(self.options).encode(),
            },
        )
    ...
    def init_turn(self, move: core.Move = None, **kwargs):
        super().init_turn(move=move)
        ...
        if self.state not in (core.GameState.NORMAL, core.GameState.CHECK):
            ...
            opponent_msg = opponent_langtable["curr-move"].format(n=self.cur_board.turn)
            ...
            if self.draw_offered:
                ...
                keyboard = [
                    ...
                    {
                        "text": player_langtable["accept-button"],
                        "data": ["ACCEPT_DRAW"],
                    },
                ]
            else:
                ...
                keyboard = [
                    {
                        "text": player_langtable["other-actions-button"],
                        "data": ["OTHER"],
                    }
                ]
                for piece in self.pieces[0]:
            ...
            if self.player_msg:
                self.player_msg = cast(
                    Message,
                    self.player_msg.edit_media(
                        media=InputMediaPhoto(
                            base.get_temp_url(
                                media.board_image(
                                    lang_code=player.language_code,
            ...
            if self.opponent_msg:
                self.opponent_msg = cast(
                    Message,
                    self.opponent_msg.edit_media(
                        media=InputMediaPhoto(
                            base.get_temp_url(
                                media.board_image(
                                    lang_code=player.language_code,
    ...
    def _send_analysis_video(self):
        if self.state in (
            ...
            text2 = "\n".join(
                [
                    base.langtable[self.player2.language_code][
                        self.state.name.lower().replace("_", "-")
                    ].format(name=self.db.get_name(player)),
                    base.langtable[self.player2.language_code]["winner"].format(
                        name=self.db.get_name(opponent)
                    ),
                    base.langtable[self.player2.language_code]["n-moves"].format(
                        n=self.cur_board.turn - 1
                    ),
                ]
            )
            ...
            text2 = "\n".join(
                [
                    ...
                    base.langtable[self.player2.language_code]["draw"],
                    base.langtable[self.player2.language_code]["n-moves"].format(
                        n=self.cur_board.turn - 1
                    ),
                ]
        ...
        try:
            ...
            video, thumb = media.board_video(self, player.language_code or opponent.language_code)
            logging.info("Analysis video created")
            ...
            if self.msg1 is not None:
                self.msg1 = cast(
                    Message,
                    self.msg1.edit_media(
                        ...
                        reply_markup=self._keyboard(
                            [
                                {
                                    "text": base.langtable[self.player1.language_code][
                                        "download-pgn"
                                    ],
                                    "data": ["DOWNLOAD", self.id],
                                }
                ...
                self.msg2 = cast(
                    ...
                    self.msg2.edit_media(
                        ...
                        reply_markup=self._keyboard(
                            [
                                {
                                    "text": base.langtable[self.player2.language_code][
                                        "download-pgn"
                                    ],
                                    "data": ["DOWNLOAD", self.id],
                                }
            ...
            text1 += (
                "\n\n"
                + base.langtable[self.player1.language_code]["visualization-error"]
            )
            text2 += (
                "\n\n"
                + base.langtable[self.player2.language_code]["visualization-error"]
            )
            if self.msg1 is not None:
                self.msg1 = cast(
                    Message,
                    self.msg.edit_caption(caption=text1),
                )
            if self.msg2 is not None:
                self.msg2 = cast(Message, self.msg2.edit_caption(caption=text2))
    ...
    def send_analysis_video(self) -> None:
        ...
        title_text2 = "\n".join(
            [
                ...
                base.langtable[self.player2.language_code]["waiting-eval"],
            ]
        )
        if self.state in (
            core.GameState.WHITE_CHECKMATED,
            ...
            title_text1 = title_text1.format(name=self.db.get_name(self.player1))
            title_text2 = title_text2.format(name=self.db.get_name(self.player2))
            ...
            self.msg2 = cast(
                ...
                self.msg2.edit_media(
                    media=InputMediaPhoto(
                        base.get_temp_url(
                            media.board_image(
                                lang_code=self.player2.language_code,
                                moves=self.moves,
                                startpos=self.startpos,
                                player1_name=self.db.get_name(self.player1),
                                player2_name=self.db.get_name(self.player2),
    ...
    def handle_input(self, command: str, args: list[str]) -> Optional[tuple[str, bool]]:
        try:
            ...
            if command == "OTHER":
                ...
                self.player_msg = cast(
                    Message,
                    cast(Message, self.player_msg).edit_caption(
                        caption="\n".join(new_text),
                        reply_markup=self._keyboard(
                            [
                                {"text": langtable["back-button"], "data": ["TURN"]},
            ...
            if command == "TURN":
                ...
                self.player_msg = cast(
                    Message,
                    cast(Message, self.player_msg).edit_media(
                        media=InputMediaPhoto(
                            base.get_temp_url(
                                media.board_image(
                                    lang_code=player.language_code,
            ...
            assert self.dispatcher is not None
            print(bool(self.player_msg), bool(self.opponent_msg), bool(self.msg1), bool(self.msg2), self.cur_board.is_white_turn)
            self.dispatcher.dispatch_error(None, exc)
            base.set_result(self.id, {})
            if self.msg1:
