...
from .analysis import eval_attack_defense_balance, ChessEngine
...
def _board_image(
    ...
    lines = get_pgn_moveseq(prev_moves, result=None, language_code="emoji", turns_per_line=1).splitlines()
    ...
    for line in reversed(lines[-5:]):
        ...
        if not y_offset and move_evaluation:
            ...
            editor.text((537, 340), move_evaluation[:2], fill="white", font=LARGE_FONT, anchor="mm")
            editor.text((552, 340), MOVE_EVAL_DESC[move_evaluation], fill="white", font=SMALL_FONT, anchor='lm')

        padded = tokens[0] + " " + tokens[1]
        if len(tokens) == 3:
            padded += " " * round((max_length - cur_line_length) // space_length + 1) + tokens[2]
        editor.text(
    ...
    if pos_evaluation is not None:
        if type(pos_evaluation) == str:
            ...
            formatted = "+" + str(pos_evaluation) if type(pos_evaluation)==float and pos_evaluation>0 else str(pos_evaluation)
            editor.text(
                (616, 400),
                formatted,
                fill="white",
            ...
            bar_x_offset =  round(math.log(abs(pos_evaluation) + 1, 10) * 47) * (1 if pos_evaluation > 0 else -1)
...
def _board_image_with_static_analysis(boards: list[BoardInfo], **kwargs):
    ...
    eval_res = eval_attack_defense_balance(boards[-1])
    for pos, score in eval_res.items():
        ...
        editor.rectangle(
            ...
            fill=tuple([round(i*255) for i in colorsys.hls_to_rgb(h, 0.75, 0.7 + abs(score) * 10)]), 
        )
...
def board_image_with_static_analysis(*args, **kwargs):
    return cv2.imencode(".jpg", _board_image_with_static_analysis(*args, **kwargs))[1].tobytes()


def board_video(match, player1_name: str = None, player2_name: str = None, analyser: ChessEngine = None) -> tuple[bytes, bytes]:
    path = os.path.join("images", "temp", match.video_filename)
    writer = cv2.VideoWriter(
        path, cv2.VideoWriter_fourcc(*"mp4v"), 15.0, BOARD.size
    )
